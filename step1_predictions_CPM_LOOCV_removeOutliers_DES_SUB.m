...
behav_vec = BehaviorData.CMAT_Subtraction_StS;
behav_vec2 = BehaviorData.CMAT_Multiplication_StS;
% behav_vec = BehaviorData.ANSes;   %% accuracy/RT

% preparing data
GrayVol = VolumeData.GrayVol;     %  132x68 
% GrayVol = VolumeData.SurfArea;     %  132x68 
% GrayVol = VolumeData.ThickAvg;
...
thresh = 0.01;
% WeightArray = zeros(size(GrayVol,1), size(GrayVol,2)+4);
...
for leftout = 1:size(GrayVol,1)
    ...
    % features
%     TrainX = [TrainingData(:,pos_nodes), AgeT1(trainIdx)', eTIVT1(trainIdx)', IQT1(trainIdx)', ones(length((trainIdx)),1)];  % 132x11
    ...
    % testing part 
%     TestX = [TestingData(:,pos_nodes), AgeT1(leftout)', eTIVT1(leftout)',  IQT1(leftout)', ones(1,1)];  % 132x11
...
SUB_GMV_prediction = [behav_vec', behav_vec_pred'];    % SUB_GMV_prediction = 130x2
csvwrite('SUB_GMV_prediction.csv',SUB_GMV_prediction);
...
save('WeightArray_sub_removeOutliers','WeightArray');
